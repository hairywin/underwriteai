...
export async function fetchNeighborhoodProfile(geo: GeocodeResult, apiKey = envCensusApiKey()): Promise<NeighborhoodProfile> {
  ...
  if (tractGeoId) {
    ...
    const tractParams = new URLSearchParams({ get: vars, for: `tract:${geo.tract!}`, in: `state:${geo.stateFips!} county:${geo.countyFips!}`, key: effectiveApiKey });
    try {
  ...
  if (countyGeoId) {
    ...
    if (cached) return cached;
    const countyParams = new URLSearchParams({ get: vars, for: `county:${geo.countyFips!}`, in: `state:${geo.stateFips!}`, key: effectiveApiKey });
